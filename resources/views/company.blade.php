...
@section('mainContent')
    ...
    <div
        ...
        <form class="space-y-3" method="post" action="{{ route('company_details_submit') }}">
            @csrf
            <h5 class="text-md font-medium text-gray-900 dark:text-white">Company Details</h5>
            ...
            <div>
                <span class="font-medium">Note:</span> If You Dont Have GST Number Continue With Individual Category
            </div>
        </div>
            @if ($errors->any())
            ...
            <div>
                <label for="gstin" class="block mb-2 text-base font-medium text-gray-900 dark:text-white">GSTIN
                    Number</label>
                <input type="gstin" name="gstin" id="gstin"
                    ...
                    placeholder="07AABCZ5840K1ZZ" value="{{ old('gstin') }}" required />
            </div>
